<template>
  <q-form @submit="onSubmit" ref="form" class="q-gutter-md">
    ...
    >{{ action === "create" ? "Nouvelle " : "Modification du " }} catégorie</div>
    <div class="row">
...
export default {
  ...
  created () {
    if (this.action === 'edit' || this.action === 'view') {
      ...
      if (this.row.category_parent !== null) {
        this.row.category_parent.label = this.row.category_parent.name
      }
      this.parent = this.row.category_parent
    }
  },
  methods: {
    onSubmit () {
          ...
          this.$axios[method](url, data).then(res => {
            if (res) {
              ...
              if (this.action === 'edit') {
                ...
                this.row.category_parent = this.parent
