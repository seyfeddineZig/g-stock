<template>
  <q-form @submit="onSubmit" ref="form" class="q-gutter-md">
    <div v-if="action !== 'view'" class="text-subtitle2 text-bold">
      {{ action === "create" ? "Nouvelle " : "Modification du " }} condition de
      règlement
    </div>
    <div class="row">
      <div class="col-md-6 q-pa-sm q-pb-xs">
        <div class="text-caption">Nom de la condition de règlement</div>
        <q-input
          filled
          dense
          ...
          :rules="[
            val =>
              (val && val.length > 0) ||
              'Veuillez saisir le nom de la condition de règlement'
          ]"
        />
      </div>
      <div class="col-md-6 q-pa-sm q-pb-xs">
        <div class="text-caption">Durée (Jours)</div>
        <q-input
          filled
          dense
          type="number"
          ...
          lazy-rules
          :rules="[
            val => (val && val.length > 0) || 'Veuillez saisir la durée'
          ]"
        />
      </div>
...
export default {
  ...
  data () {
    return {
      moduleName: 'paymentcondition',
      moduleLabel: 'La condittion de règlement',
      name: '',
      description: '',
      duration: null,
      active: true
    ...
    onSubmit () {
      this.$refs.form.validate().then(success => {
        if (success) {
          ...
          let url = 'paymentcondition/create'
          let method = 'post'
          let action = 'crée'
          if (this.action === 'edit') {
