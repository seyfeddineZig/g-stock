<template>
  <q-form @submit="onSubmit" ref="form" class="q-gutter-md">
    <div v-if="action !== 'view'" class="text-subtitle2 text-bold">
      {{ action === "create" ? "Nouveau " : "Modification du " }} département
    </div>
    <div class="row">
      <div class="col-md-6 q-pa-sm q-pb-xs">
        <div class="text-caption">Nom de département</div>
        <q-input
          filled
          dense
          ...
          :rules="[
            val =>
              (val && val.length > 0) || 'Veuillez saisir le nom de département'
          ]"
        />
      </div>
      <div class="col-md-6 q-pa-sm q-pb-xs">
        <div class="text-caption">Code</div>
        <q-input
          filled
          dense
          :readonly="action === 'view' ? true : false"
...
export default {
  ...
  computed: {
    countries () {
      ...
      countries.forEach(el => (el.label = el.name))
      return countries
    }
  },
  created () {
